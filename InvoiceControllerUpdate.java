...
@WebServlet(description = "/", urlPatterns = { "/update-invoice" })
public class InvoiceControllerUpdate extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String param = request.getParameter("inumber");
		if(param.trim().equals("")){
			param="-1";
		}
		
		HttpSession session = request.getSession();
		session.setAttribute("invoiceById", invoiceService.getInvoiceById(param));

		response.sendRedirect("ccinvoice.jsp?update=true");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
				...
				invoicelService.updateInvoice(
						Integer.parseInt(cinumber), 
						ds,
