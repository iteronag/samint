<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
	pageEncoding="ISO-8859-1"%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<title>International </title>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<link rel="stylesheet"
	href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css">
<script
	src="https://ajax.googleapis.com/ajax/libs/jquery/3.1.1/jquery.min.js"></script>
<script
	src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js"></script>
<!-- Include Date Range Picker -->
<script type="text/javascript" src="https://cdnjs.cloudflare.com/ajax/libs/bootstrap-datepicker/1.4.1/js/bootstrap-datepicker.min.js"></script>
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/bootstrap-datepicker/1.4.1/css/bootstrap-datepicker3.css"/>

	
<script>
    $(document).ready(function(){
      var date_input=$('input[name="idate"]'); //our date input has the name "date"
      var container=$('.container form').length>0 ? $('.container form').parent() : "body";
      var options={
        format: 'mm/dd/yyyy',
        container: container,
        todayHighlight: true,
        autoclose: true,
      };
      date_input.datepicker(options);
    })
</script>
	
</head>
<body>
	<div class="container">

		<c:choose>
			<c:when test="${param.update == 'true'}">
				<h2>Update Invoice Information</h2>
				<form method="post" action="update-invoice" class="form-inline">
					<c:forEach items="${invoiceById}" var="val">
						<input type="hidden" name="id" value="${val.key}">
						
						
					<div class="row">
				    <div class="col-sm-3"><label for="inumber">Invoice Number:</label></div>
				    <div class="col-sm-3">
						<input type="text"
						class="form-control" id="inumber" name="inumber"
						value="${val.value.cinumber}">				    
				    </div>
				
      				<div class="col-sm-3"><label for="idate">Invoice Date:</label></div>
      				<div class="col-sm-3"> <!-- Date input -->
        				<input type="text" 
        				class="form-control" id="idate" name="idate" 
        	   			value="${val.value.idate}" />
      				</div>				        
				  </div>
		
				  <div class="row">
				    <div class="col-sm-3"><label for="cnumber">Customer Number:</label></div>
				    <div class="col-sm-3">
						<input type="cnumber"
						class="form-control" id="cnumber" name="cnumber"
						value="${val.value.cnumber}">				    
				    </div>
				    <div class="col-sm-3"><label for="cname">Customer Name:</label></div>
				    <div class="col-sm-3">
						<input type="cname"
						class="form-control" id="cname" name="cname"
						value="${val.value.cname}">				    
				    </div>				    
				  </div>

				  <div class="row">
				    <div class="col-sm-3"><label for="qref">Quotation Ref:</label></div>
				    <div class="col-sm-3">
						<input type="text"
						class="form-control" id="qref" name="qref"
						value="${val.value.qref}">
						
						
						
				    </div>
				    <div class="col-sm-3"><label for="currency">Currency:</label></div>
				    <div class="col-sm-3">
						<input type="text"
						class="form-control" id="currency" name="currency"
						value="${val.value.currency}">				    
				    </div>				    
				  </div>

				  <div class="row">
				    <div class="col-sm-3"><label for="iamount">Invoice Amount:</label></div>
				    <div class="col-sm-3">
						<input type="text"
						class="form-control" id="iamount" name="iamount"
						value="${val.value.iamount}">				    
				    </div>

				    <div class="col-sm-3"><label for="pcurrency">Parallel Currency:</label></div>
				    <div class="col-sm-3">
						<input type="text"
						class="form-control" id="pcurrency" name="pcurrency"
						value="${val.value.pcurrency}">				    
				    </div>
				    				    
				  </div>

				  <div class="row">
				    <div class="col-sm-3"><label for="bname">Bank Name:</label></div>
				    <div class="col-sm-3">
						<input type="text"
						class="form-control" id="bname" name="bname"
						value="${val.value.bname}">				    
				    </div>

				    <div class="col-sm-3"><label for="erate">Exchange Rate:</label></div>
				    <div class="col-sm-3">
						<input type="text"
						class="form-control" id="erate" name="erate"
						value="${val.value.erate}">				    
				    </div>
				  </div>

				  <div class="row">
				    <div class="col-sm-3"><label for="baccount">Bank Account:</label></div>
				    <div class="col-sm-3">
						<input type="text"
						class="form-control" id="baccount" name="baccount"
						value="${val.value.baccount}">				    
				    </div>
				    
						
						<button type="submit" class="btn btn-default" value="Update">Update</button>
					</c:forEach>
				</form>

			</c:when>
			<c:otherwise>
				<h2>Create Customer Invoice</h2>
				<form method="post" action="add-invoice" class="form-inline">
		
				  <div class="row"> <!-- 
				    <div class="col-sm-3"><label for="inumber">Invoice Number:</label></div>
				    <div class="col-sm-3">
						<input type="inumber"
						class="form-control" id="inumber" name="inumber"
						placeholder="Enter Invoice Number">				    
				    </div>
				 -->
      				<div class="col-sm-3"><label for="idate">Invoice Date:</label></div>
      				<div class="col-sm-3"> <!-- Date input -->
        				<input type="text" 
        				class="form-control" id="idate" name="idate" 
        	   			placeholder="MM/DD/YYY" />
      				</div>				        
				  </div>
		
				  <div class="row">
				    <div class="col-sm-3"><label for="cnumber">Customer Number:</label></div>
				    <div class="col-sm-3">
						<input type="cnumber"
						class="form-control" id="cnumber" name="cnumber"
						placeholder="Enter Customer No">				    
				    </div>
				    <div class="col-sm-3"><label for="cname">Customer Name:</label></div>
				    <div class="col-sm-3">
						<input type="cname"
						class="form-control" id="cname" name="cname"
						placeholder="Enter Customer Name">				    
				    </div>				    
				  </div>

				  <div class="row">
				    <div class="col-sm-3"><label for="qref">Quotation Ref:</label></div>
				    <div class="col-sm-3">
						<input type="text"
						class="form-control" id="qref" name="qref"
						placeholder="Enter Quotation Reference">
										    
				    </div>
				    <div class="col-sm-3"><label for="currency">Currency:</label></div>
				    <div class="col-sm-3">
						<input type="text"
						class="form-control" id="currency" name="currency"
						placeholder="currency">				    
				    </div>				    
				  </div>

				  <div class="row">
				    <div class="col-sm-3"><label for="iamount">Invoice Amount:</label></div>
				    <div class="col-sm-3">
						<input type="text"
						class="form-control" id="iamount" name="iamount"
						placeholder="Enter Invoice Amount">				    
				    </div>

				    <div class="col-sm-3"><label for="pcurrency">Parallel Currency:</label></div>
				    <div class="col-sm-3">
						<input type="text"
						class="form-control" id="pcurrency" name="pcurrency"
						placeholder="Enter Parallel Currency">				    
				    </div>
				    				    
				  </div>

				  <div class="row">
				    <div class="col-sm-3"><label for="bname">Bank Name:</label></div>
				    <div class="col-sm-3">
						<input type="text"
						class="form-control" id="bname" name="bname"
						placeholder="Enter Bank Name">				    
				    </div>

				    <div class="col-sm-3"><label for="erate">Exchange Rate:</label></div>
				    <div class="col-sm-3">
						<input type="text"
						class="form-control" id="erate" name="erate"
						placeholder="Enter Exchange Rate ">				    
				    </div>
				  </div>

				  <div class="row">
				    <div class="col-sm-3"><label for="baccount">Bank Account:</label></div>
				    <div class="col-sm-3">
						<input type="text"
						class="form-control" id="baccount" name="baccount"
						placeholder="Enter Bank Account ">				    
				    </div>
				    
				    <div class="col-sm-3"></div>
				    <div class="col-sm-3"></div>				    
				  </div>
	

				   <div class="row">
				   <div class="col-sm-3"></div>
				   <div class="col-sm-3"></div>
				   </div>			
				   <br>	  
				   <div class="row">
				   <div class="col-sm-3"></div>
				   <div class="col-sm-3">
					<button type="submit" class="btn btn-default">Save</button>
									  		  
						<a href="home.jsp" class="btn btn-default">	Cancel	</a>
					
					<button type="submit" class="btn btn-default">Search</button>
					
					
					</div>					
					</div>
					 										
				</form>
			</c:otherwise>
		</c:choose>
		
<h2>Quotation Materials</h2>
<form action="search-invoice_material">
<table>
<tr><td>
<input type="text" 
class="form-control" id="qref" name="qref"
placeholder="Enter Quotation Reference">
</td>
<td><button type="submit" class="btn btn-info">Get Materials</button></td></tr>
</table>
</form>
				<table class="table table-striped">
				<thead>
					<tr>
						<th>S.No</th>
						<th>Material </th>
						<th>Material Desc</th>
						<th>UOM</th>
						<th>Quantity</th>
						<th>Rate</th>
						<th>Amount</th>
						<th>Account No</th>
						<th>Remarks</th>
						<th>Status</th>
					</tr>
				</thead>
				<tbody>
						<c:forEach items="${quoMaterialListForInvoice}" var="val">
							<tr>
								<td></td>
								<td></td>
								<td>
								<a href="#" >
									<i> <c:out value="${val.value.mdesc}" />
									</i> </a>
</td>
								<td>
								<c:out value="${val.value.uom}" />
								</td>
								<td><c:out value="${val.value.quantity}" /></td>
								<td></td>
								<td><c:out value="${val.value.netprice}" /></td>
								<td><!-- 
								<a href="update-contact?id=${val.key}"
									class="btn btn-info" role="button">Update</a>  -->
									</td>
								<td><!-- 
								<a href="delete-mat-quotation?id=${val.key}"
									class="btn btn-info" role="button">Delete</a>  -->
									<c:out value="${val.value.remarks}" />
									</td>
									<td></td>
							</tr>
						</c:forEach>
				</tbody>
				</table>
				<p><!-- 
				<form action="reports.jsp">
						<button type="submit" class="btn btn-default">Back</button>
				</form>
				 --> </p>
				 		
</body>
</html>